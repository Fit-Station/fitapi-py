...
branchId = "AB9820F9-6F57-4808-A1FA-76CCC7B8AAD5"
deviceId = "C49045A2-2C62-417B-8436-CD57A9CB2DDB"
#Canik
global yon1_pin
global yon2_pin
global yon1
...
filename = PhotoImage(file="/home/arastatek/Bookshelf/Bookshelf/yeniFitStation.png")
...
def TurnstyleTurn(direction):
    try:
        if isSerial == 1:
            ...
            if direction == 0:  # giris
                Port.write(str.encode('+Tur_ba-'))
            elif direction == 1:  # cikis
                Port.write(str.encode('+Tur_ab-'))
            elif direction == 2:  # cikis serbest
                Port.write(str.encode('+Tur_as-'))
            elif direction == 3:  # giris serbest
                Port.write(str.encode('+Tur_bs-'))
            elif direction == 4:  # cikis kilitli
                Port.write(str.encode('+Tur_ak-'))
            elif direction == 5:  # giris kilitli
                Port.write(str.encode('+Tur_bk-'))
            elif direction == 6:  # hepsi kilitli
                Port.write(str.encode('+Tur_hk-'))
            elif direction == 7:  # hepsi serbest
                Port.write(str.encode('+Tur_hs-'))
            elif direction == 8:  # sistemrestart
    ...
    try:
        if ali["isSuccess"]:
            ...
            try:
                try:
                    # if pathOfImage == "":
                    #     test = PhotoImage(file="tanimsiz.png")
                    # elif pathOfImage == None:
                    #     test = PhotoImage(file="tanimsiz.png")
                    # else:
...
def SendExceptionInfo(e):
    try:
        ...
        SonucDeger.config(text="Hata oluÅŸtu")
        SonucDeger.after(beklemeSuresi, lambda: SonucDeger.config(text=""))
